...
import { addAppointmentNotes, createPrescription, orderMedication } from './actions'
...
export function DashboardClient({
  ...
  // doctor modals
  const [notesModal, setNotesModal] = useState<{ open: boolean; appointmentId: string; patientName: string } | null>(null)
  const [prescriptionModal, setPrescriptionModal] = useState<{ open: boolean; patientId: string; patientName: string } | null>(null)
  ...
  const pastAppointments = patientAppointments.filter(apt => new Date(apt.date) < new Date()).slice(0, 5)
  ...
  async function handleOrderMedication(prescriptionId: string, medicationName: string) {
  ...
  return (
    <div className="container mx-auto py-8 px-4 space-y-8">
          ...
          <Card className="relative z-10 w-full max-w-lg mx-4">
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setNotesModal(null)} className="text-gray-400 hover:text-gray-600 hover:cursor-pointer">
                  <X className="h-5 w-5" />
                </button>
              </div>
              <CardDescription>Patient: {notesModal.patientName}</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <Button onClick={handleAddNotes} disabled={isSubmitting} className="w-full hover:cursor-pointer">
      ...
      {prescriptionModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/50" onClick={() => setPrescriptionModal(null)} />
          <Card className="relative z-10 w-full max-w-lg mx-4">
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle>Create Prescription</CardTitle>
                <button onClick={() => setPrescriptionModal(null)} className="text-gray-400 hover:text-gray-600 hover:cursor-pointer">
                  <X className="h-5 w-5" />
                </button>
              </div>
              <CardDescription>For: {prescriptionModal.patientName}</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setPrescriptionData(p => ({ ...p, medication: e.target.value }))}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setPrescriptionData(p => ({ ...p, dosage: e.target.value }))}
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) => setPrescriptionData(p => ({ ...p, instructions: e.target.value }))}
                  placeholder="Take with food. Complete the full course..."
                  rows={3}
                />
              </div>
              <Button onClick={handleCreatePrescription} disabled={isSubmitting} className="w-full bg-blue-600 hover:bg-blue-700 hover:cursor-pointer">
                {isSubmitting ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <FilePlus2 className="h-4 w-4 mr-2" />}
                Create Prescription
              </Button>
            </CardContent>
          </Card>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-blue-900">Welcome back, {userName}</h1>
                ...
                {doctorAppointmentsToday.map((apt) => (
                  <Card key={apt.id} className="hover:shadow-lg transition-shadow">
                    <CardContent className="p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                      <Link href={`/appointments/${apt.id}`} className="flex items-start gap-4 flex-1 hover:cursor-pointer">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-3 text-sm text-gray-500 mt-2">
                            ...
                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                              apt.status === 'completed' ? 'bg-green-50 text-green-700' : 'bg-yellow-50 text-yellow-700'
                            }`}>
                              {apt.status}
                            </span>
                          </div>
              ...
              {doctorAppointments.length === 0 ? (
                <Card className="bg-gray-50 border-dashed">
                  <CardContent className="py-8 text-center text-gray-500">
                    No appointments yet. Patients will appear here once they book.
                  </CardContent>
                </Card>
              ) : (
                  ...
                  {doctorAppointments.map((apt) => (
                    <Card key={apt.id} className="hover:shadow-md transition-shadow">
                      <CardContent className="p-4 flex items-center justify-between">
                        <Link href={`/appointments/${apt.id}`} className="flex items-center gap-3 flex-1 hover:cursor-pointer">
                ...
                <CardContent className="p-4 space-y-3">
                  {doctorQueue.slice(0, 5).map((apt) => (
                    <div key={apt.id} className="flex items-center justify-between">
                      <div>
                        <p className="font-medium text-gray-900">{apt.patient?.full_name || 'Patient'}</p>
                        <p className="text-xs text-gray-500">{new Date(apt.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                      </div>
            ...
            <Card className="bg-blue-600 text-white border-none">
              ...
              <CardContent className="space-y-3">
                <div className="flex justify-between">
                  <span className="text-blue-100">Today&apos;s appointments</span>
                  <span className="font-bold">{doctorAppointmentsToday.length}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-blue-100">Pending patients</span>
                  <span className="font-bold">{doctorQueue.length}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-blue-100">Completed today</span>
                  <span className="font-bold">{doctorAppointmentsToday.filter(a => a.status === 'completed').length}</span>
                </div>
              </CardContent>
            </Card>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            {upcomingPatientAppointments.length === 0 ? (
              <Card className="bg-gray-50 border-dashed">
                <CardContent className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <Link href="/book">
                    <Button variant="outline" className="hover:cursor-pointer">Find a Doctor</Button>
                  </Link>
                </CardContent>
              </Card>
            ) : (
              <div className="space-y-4">
                {upcomingPatientAppointments.map((apt) => (
                  <Link key={apt.id} href={`/appointments/${apt.id}`} className="block hover:cursor-pointer">
                ...
                <CardContent className="p-6">
                  {prescriptions.filter(p => p.status === 'active').length === 0 ? (
                    <div className="text-center py-6">
                      ...
                      <p className="text-gray-500">No active prescriptions to order</p>
                      <p className="text-sm text-gray-400 mt-1">Book an appointment to get prescriptions</p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {prescriptions.filter(p => p.status === 'active').map((script) => {
                        const alreadyOrdered = medicationOrders.some(o => o.medication_name === script.medication_name && o.status !== 'delivered')
                        return (
                          <div key={script.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="font-semibold text-gray-900">{script.medication_name}</p>
                                <p className="text-sm text-gray-500">{script.dosage}</p>
                                {script.instructions && (
                                  <p className="text-xs text-gray-400 mt-1">{script.instructions}</p>
                                )}
                              </div>
                            </div>
                            {alreadyOrdered || orderedMeds.includes(script.medication_name) ? (
                              ...
                              <Button
                                onClick={() => handleOrderMedication(script.id, script.medication_name)}
                                disabled={orderingMed === script.medication_name}
                      ...
                      return (
                        <div key={order.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-semibold text-gray-900">{order.medication_name}</p>
            ...
            <div className="mt-8">
              ...
              <Card>
                <CardContent className="p-6">
                  {pastAppointments.length === 0 && prescriptions.length === 0 ? (
                    <div className="text-center py-6">
                    ...
                    <div className="space-y-4">
                      {pastAppointments.length > 0 && (
                        <div>
                          <h3 className="text-sm font-semibold text-gray-700 mb-3">Past Consultations</h3>
                          <div className="space-y-2">
                            {pastAppointments.map((apt) => (
                              <div key={apt.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                <div>
                                  <p className="font-medium text-gray-900">{apt.doctors.name}</p>
                                  <p className="text-sm text-gray-500">{apt.doctors.specialty}</p>
                  ...
                  <Card key={script.id}>
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between">
                        <div>
                          <p className="font-bold text-gray-900">{script.medication_name}</p>
                          <p className="text-sm text-gray-600">{script.dosage}</p>
                        </div>
                        <Badge className={script.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}>
                          {script.status}
                        </Badge>
                      </div>
                    </CardContent>
            ...
            <Card className="border-orange-200 bg-orange-50">
              <CardHeader>
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-base text-orange-800">Reminders</CardTitle>
                </div>
              </CardHeader>
              <CardContent className="space-y-3">
                {upcomingPatientAppointments.length === 0 && refillReminders.length === 0 ? (
                  <p className="text-sm text-orange-700">No upcoming reminders</p>
                ) : (
                  <>
                    {upcomingPatientAppointments.slice(0, 2).map((apt) => (
                      <div key={apt.id} className="flex items-start gap-2 text-sm">
                        <Calendar className="h-4 w-4 text-orange-600 mt-0.5 shrink-0" />
                        <div>
                          <p className="font-medium text-orange-800">Appt: {apt.doctors.name}</p>
                          <p className="text-orange-600">{new Date(apt.date).toLocaleDateString()}</p>
                        </div>
                      </div>
                    ))}
                    {refillReminders.map((script) => (
                      <div key={script.id} className="flex items-start gap-2 text-sm">
                        <Pill className="h-4 w-4 text-orange-600 mt-0.5 shrink-0" />
                        <div>
                          <p className="font-medium text-orange-800">{script.medication_name}</p>
                          <p className="text-orange-600">Refill soon</p>
                        </div>
                      </div>
                    ))}
                  </>
